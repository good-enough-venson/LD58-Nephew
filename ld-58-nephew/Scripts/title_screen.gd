...
func try_rank_up() -> void:
	if PocketInventory.remove_stones(Running.winCondition.duplicate()):
		...
		SfxManager.on_congratulate()
		SfxManager.on_rank_up()
...
func get_hint_stone_family(stone:Stone, access:int) -> Dictionary:
	var hint = {"text":"", "callback":func() -> void: pass}
	match access:
		0,1: 
			hint.text = "..it seems this stone belongs to the %s arcana... 
			all such runestones have the same catalystic function.. yes."
			hint.text = hint.text % [stone.catalyst.name]
			hint.callback = func() -> void: Running.hint_new_arcana(stone)
		2:
			hint.text = "ah, yes.. a %s stone. it %s."
			hint.text = hint.text % [stone.catalyst.name, stone.catalyst.note]
			hint.callback = func() -> void: Running.hint_new_arcana(stone)
		_:
			hint.text = "..you probably kow that the %s arcana %s."
			hint.text = hint.text % [stone.catalyst.name, stone.catalyst.description]
			hint.callback = func() -> void: Running.hint_new_arcana(stone)
	return hint
	
func get_hint_stone_power(stone:Stone, access:int) -> Dictionary:
	var hint = {"text":"", "callback":func() -> void: pass}
	match access:
		0,1: 
			hint.text = "..a runestone's base power is different from it's rune value. This rune is a %d."
			hint.text = hint.text % Stone.get_stone_sibId(stone.value)
			hint.callback = func(e) -> void: Running.hint_new_rune(stone)
			#print(stone.sibId)
		2:
			hint.text = "this runestone's base power is... ..ah, yes. it resonates nicely in %d."
			hint.text = hint.text % stone.value
		3:
			hint.text = "the primary runestone arcana resonate on unique frequencies.. this stone's arcana resonates in %d"
			hint.text = hint.text % [Stone.get_root_stone(stone.value)]
		_:
			hint.text = "to estimate a rune stone's base power, just multiply the rune value(%d) by the arcana resonance(%d), getting %d"
			hint.text = hint.text % [stone.sibId, Stone.get_root_stone(stone.value), stone.value]
			hint.callback = func() -> void: Running.hint_new_rune(stone)
	return hint
